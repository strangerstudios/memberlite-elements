 ...
 */
function memberlite_elements_recent_posts_shortcode_handler($atts, $content=null, $code="") {
	global $post;
	...
	if( !empty($post_type) && post_type_exists( $post_type ) ) {
		...
		return _e( "There was a problem fetching content for the post type '$post_type'. Please ensure this exists.", "memberlite-elements" );
	}
	...
	if( $author_id === 'current' && !empty( $author_id ) ) {
		...
		if( empty( $current_user->ID) ) {
			return _e( 'There was a problem fetching posts for the current user. Please try again later.', 'memberlite-elements' );
		}else{
	...
	if( $author_id != 'current' && !is_numeric( $author_id ) && !empty( $author_id ) ) {
		...
		if( empty( $author_id ) ) {
			return __( "No posts found for this user. Please ensure the author's name/ID is correct.", 'memberlite-elements' );
		}else{
...
add_shortcode('memberlite_recent_posts', 'memberlite_elements_recent_posts_shortcode_handler');
